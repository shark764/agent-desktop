...
import { selectCheckedContacts, selectNextNotificationId } from 'containers/InfoTab/selectors';
import { selectAttributes } from 'containers/ContactSearchBar/selectors';
import { addNotification, dismissNotification } from 'containers/InfoTab/actions';
import { setContactMode, initForm, setFormValidity, setEditingContacts, setContactSaveLoading, assignContact, resetForm, removeSearchFilter } from 'containers/AgentDesktop/actions';
import { selectInteractionsList, selectNoInteractionContactPanel, selectNewInteractionContactPanel } from 'containers/AgentDesktop/selectors';
import { addContactNotification as addContactNotificationAction, addContactErrorNotification as addContactErrorNotificationAction } from './actions';
import {
  EDIT_CONTACT,
  MERGE_CONTACTS,
  NEW_CONTACT,
...
export function* getInteraction(interactionId) {
  ...
  if (!interactionId) {
    ...
    const interactionMap = interactionsList.find((listInteraction) => listInteraction.get('interactionId') === interactionId);
...
export function* goEditContact(action) {
  const contactAttributes = action.contact && action.contact.attributes ? action.contact.attributes : {};
  const layoutSections = yield select(selectPopulatedLayout);
  const contactForm = {};
  const formErrors = {};
  const showErrors = {};
  for (let sectionIndex = 0; sectionIndex < layoutSections.length; sectionIndex += 1) {
  ...
  yield put(initForm(action.interactionId, contactForm, formErrors, showErrors));
  yield put(setEditingContacts(action.interactionId, [action.contact || {}]));
  yield put(setContactMode(action.interactionId, action.contact ? 'edit' : 'create'));
}
...
export function* goMergeContacts(action) {
  ...
  for (let valueIndex = 0; valueIndex < values.length; valueIndex += 1) {
    ...
    const attribute = attributes.find((attr) => attr.objectName === attributeName);
    if (firstContact[attributeName] === undefined || firstContact[attributeName] === '') {
      contactForm[attributeName] = secondContact[attributeName] || attribute.default || '';
      formErrors[attributeName] = getError(attribute, secondContact[attributeName]);
    } else {
      contactForm[attributeName] = firstContact[attributeName];
      formErrors[attributeName] = getError(attribute, firstContact[attributeName]);
  ...
  yield put(setEditingContacts(action.interactionId, [checkedContacts[0], checkedContacts[1]]));
  yield put(initForm(action.interactionId, contactForm, formErrors, showErrors, unusedFields, selectedIndexes));
...
export function* goAddContactNotification(action) {
  ...
  yield put(addNotification({ ...action.notificationInfo, id: notificationId, isError: false }));
...
export function* goAddContactErrorNotification(action) {
  const notificationId = yield select(selectNextNotificationId);
  yield put(addNotification({ ...action.notificationInfo, id: notificationId, isError: true }));
}
...
export function* goSubmitContactCreate(action) {
  ...
  try {
    ...
    console.error(error);
    yield put(setContactSaveLoading(action.interactionId, false));
    yield put(addContactErrorNotificationAction({ errorType: 'serverError', messageType: 'notCreated' }));
  }
}
...
export function* goSubmitContactEdit(action) {
  ...
  try {
    ...
    const editedAssigned = (
      interaction.contact
      && originalContacts[0].id === interaction.contact.id
    );
    yield put(setContactMode(action.interactionId, editedAssigned ? 'view' : 'search'));
    yield put(setContactSaveLoading(action.interactionId, false));
    yield put(resetForm(action.interactionId));
  } catch (error) {
    console.error(error);
    yield put(setContactSaveLoading(action.interactionId, false));
    yield put(addContactErrorNotificationAction({ errorType: 'serverError', messageType: 'notSaved' }));
  }
}
...
export function* goSubmitContactMerge(action) {
  ...
  try {
    const createdContact = yield call(
      ...
      { attributes: contactForm, contactIds: originalContacts.map((contact) => contact.id) },
      'ContactsControl'
    );
    ...
    console.error(error);
    yield put(setContactSaveLoading(action.interactionId, false));
    yield put(addContactErrorNotificationAction({ errorType: 'serverError', messageType: 'notMerged' }));
  }
}
...
export function* addContactErrorNotification() {
  yield takeEvery(ADD_CONTACT_ERROR_NOTIFICATION, goAddContactErrorNotification);
}
